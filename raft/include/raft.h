...
public: 
    ...
    int GetPreLogTerm(int id){
        if(nextIndex[id]-1<0)return -1;
        unique_lock<mutex> lock(mtx);
        return log[nextIndex[id]-1].m_term;
    }
    int GetCommitIndex(){
        unique_lock<mutex>lock(commit_mtx);
        return commitIndex;
    }
    int SetCommitIndex(int index){
        unique_lock<mutex>lock(commit_mtx);
        int tmp=commitIndex;
        commitIndex=index;
        if(index!=tmp){
    ...
    std::vector<LogEntry> GetEntries(int index){//获取对应长度的log
        ...
        for(auto entry=log.begin()+index; entry!=log.end();entry++){
    ...
    std::vector<LogEntry> GetEntries(int index,int index_end){//获取对应长度的log,重载
        unique_lock<mutex> lock(log_mtx);
        vector<LogEntry> outentries; 
        if(index>index_end)return outentries;
        for(auto entry=log.begin()+index; entry!=log.begin()+index_end&&entry!=log.end();entry++){
            outentries.push_back(*entry);
        }
        return outentries;
    }
    void RollLog(const int size){
        unique_lock<mutex> lock(mtx);
