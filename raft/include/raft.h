...
public: 
    ...
    int GetIndexTerm(int index){
        ...
        cout<<" index_log "<<index_log<<" state_machine->GetIndex()"<<state_machine->GetIndex()<<"log size "<<log.size()<<endl;
        unique_lock<mutex>lock(log_mtx);
        if(log.size()==0){
            return state_machine->GetLastTerm();
        }else{
    ...
    int GetPreLogTerm(int id){
        ...
        cout<<"GetPreLogTerm: "<<state_machine->GetLastTerm()<<endl;
        {
            unique_lock<mutex>lock(log_mtx);
    ...
    std::vector<LogEntry> GetEntries(int index){//获取对应长度的log
        int index_log=index-state_machine->GetIndex();
        unique_lock<mutex> lock(log_mtx);
        if(log.size()>0)cout<<" log[0] index"<<log[0].m_index;
        cout<<" Node "<<GetMyId()<<" "<<index_log<<" "<<index<<" state_machine "<<state_machine->GetIndex()<<" logsize "<<log.size()<<endl;
